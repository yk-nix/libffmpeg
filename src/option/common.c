...
static void (*show_help_default)(const char *opt, const char *arg) = NULL;
static int program_birth_year = 1971;
...
void program_init(int prog_birth_year, const char *prog_name, void (*show_help_default_func)(const char *opt, const char *arg)) {
	program_name = prog_name;
	program_birth_year = prog_birth_year;
	show_help_default = show_help_default_func;
}
...
int show_sources(void *optctx, const char *opt, const char *arg) {
	...
	do {
		fmt = av_input_audio_device_next((AVInputFormat *)fmt);
		if (fmt) {
	...
	do {
		fmt = av_input_video_device_next((AVInputFormat *)fmt);
		if (fmt) {
			if (dev && !av_match_name(dev, fmt->name))
				continue;
			print_device_sources(fmt, opts);
...
int show_sinks(void *optctx, const char *opt, const char *arg) {
	...
	do {
		fmt = av_output_audio_device_next((AVOutputFormat *)fmt);
		if (fmt) {
			if (dev && !av_match_name(dev, fmt->name))
				continue;
	...
	do {
		fmt = av_output_video_device_next((AVOutputFormat *)fmt);
		if (fmt) {
			if (dev && !av_match_name(dev, fmt->name))
				continue;
			print_device_sinks(fmt, opts);
